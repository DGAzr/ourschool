...
class PointTransactionBase(BaseModel):
    ...
    amount: int = Field(
        ..., description="Points amount (positive for awards, negative for deductions)"
    )
    ...
    source_description: Optional[str] = Field(
        None, description="Brief description of the source"
    )
    notes: Optional[str] = Field(
        None, description="Detailed notes about the transaction"
    )
...
class AdminPointAdjustment(BaseModel):
    ...
    notes: str = Field(
        ..., min_length=1, max_length=500, description="Reason for the adjustment"
    )
...
class StudentPointsBase(BaseModel):
    ...
    total_spent: int = Field(
        default=0, description="Total points spent/deducted lifetime"
    )
...
class StudentPointsWithTransactions(StudentPoints):
    ...
    recent_transactions: List[PointTransaction] = Field(
        default_factory=list, description="Recent point transactions"
    )
...
class PointsSystemStatus(BaseModel):
    ...
    can_toggle: bool = Field(
        True, description="Whether the current user can toggle this setting"
    )
